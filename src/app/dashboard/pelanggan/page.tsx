...
export default function PelangganPage() {
  ...
  const filteredData = useMemo(() => {
    return pelanggan.filter(p => {
      let passTipe = jenisFilter === "Semua" || p.jenis === jenisFilter
      let passSearch = !searchQuery || p.nama.toLowerCase().includes(searchQuery.toLowerCase())
  ...
  const counts = useMemo(() => {
    ...
    pelanggan.forEach(p => {
      // safely increment even if typo exists in db
  ...
  return (
    <>
      <header className="sticky top-0 z-10 flex items-center justify-between px-8 py-5 bg-[#0d1117]/80 backdrop-blur border-b border-white/[0.05]">
        <div>
          <h2 className="text-2xl font-bold text-white tracking-tight">Master Pelanggan</h2>
          <p className="text-sm text-gray-500 mt-1">Kelola data seluruh pelanggan, agen, dan retail Anda</p>
        </div>
        <button 
          onClick={() => handleOpenModal()}
          className="flex items-center gap-2 bg-green-600 hover:bg-green-500 text-white text-sm font-semibold px-5 py-2.5 rounded-xl transition-all shadow-lg shadow-green-900/20"
        >
      ...
      <main className="p-8 space-y-8">
        ...
        <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
          ...
          {TIPE_OPTIONS.map(opt => (
            <div key={opt} className="bg-[#161b22] border border-white/[0.05] rounded-2xl p-5 min-w-[160px] flex-1">
            ...
            {isLoading ? (
              ...
              <table className="w-full text-left text-sm text-gray-300">
                <thead className="text-xs text-gray-400 uppercase bg-[#0d1117]/80 border-b border-white/[0.05]">
                  <tr>
                    <th scope="col" className="px-6 py-4 font-medium">Nama</th>
                    <th scope="col" className="px-6 py-4 font-medium">Tipe</th>
                    <th scope="col" className="px-6 py-4 font-medium">Telepon</th>
                    <th scope="col" className="px-6 py-4 font-medium">Alamat</th>
                    <th scope="col" className="px-6 py-4 font-medium">Catatan</th>
                    ...
                    <tr key={p.id} className="hover:bg-white/[0.01] transition-colors group">
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className="font-semibold text-white text-base">{p.nama}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-center">
                        <button 
                          ...
                          className="bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 px-4 py-2 rounded-lg text-xs font-semibold transition-all border border-blue-500/20 opacity-0 group-hover:opacity-100"
                        >
                          Edit ✏️
                        </button>
                      </td>
            ...
            <div className="p-6 overflow-y-auto space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="w-full bg-[#0d1117] border border-white/10 text-white rounded-md px-3 py-2 flex h-10 text-sm focus-visible:outline-none focus:ring-1 focus:ring-green-500"
                >
                  {TIPE_OPTIONS.map(opt => (
                    <option key={opt} value={opt}>{opt}</option>
                  ))}
              ...
              <div className="space-y-1.5">
                ...
                <textarea 
                  ...
                  className="w-full bg-[#0d1117] border border-white/10 text-white rounded-md px-3 py-2 text-sm focus-visible:outline-none focus:ring-1 focus:ring-green-500 min-h-[80px]"
              ...
              <div className="space-y-1.5">
                ...
                <Input 
                  ...
                  className="bg-[#0d1117] border-white/10 text-white"
                  placeholder="Punya alergi telat bayar..."
                />
              </div>
            </div>
