...
export function ProdukTab() {
  ...
  const [formData, setFormData] = useState({ nama: "", satuan: "kg", aktif: true })
  ...
  const fetchProduk = async () => {
    ...
    try {
      ...
      if (!error && data) setProduk(data as Produk[])
      else if (error) console.error("Error fetching", error)
    } catch (e) { console.error(e) }
    finally { setIsLoading(false) }
  }

  const handleOpenModal = (p?: Produk) => {
    if (p) {
      setEditingId(p.id)
      setFormData({ nama: p.nama || "", satuan: p.satuan || "kg", aktif: p.aktif ?? true })
    } else {
      setEditingId(null)
      setFormData({ nama: "", satuan: "kg", aktif: true })
    }
    setIsModalOpen(true)
  }
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        const { data, error } = await supabase.from('produk').update(formData).eq('id', editingId).select().single()
        if (error) throw error
        setProduk(prev => prev.map(p => p.id === editingId ? data : p))
      } else {
        const { data, error } = await supabase.from('produk').insert(formData).select().single()
        if (error) throw error
        setProduk(prev => [...prev, data].sort((a, b) => a.nama.localeCompare(b.nama)))
      }
  ...
  const filteredData = useMemo(() => {
    return produk.filter(p => !searchQuery || p.nama.toLowerCase().includes(searchQuery.toLowerCase()))
  }, [produk, searchQuery])
        ...
        <div className="overflow-x-auto min-h-[400px]">
          {isLoading ? (
            ...
            <div className="p-20 text-center text-gray-500 flex flex-col items-center">
              ...
              <p className="text-lg">Tidak ada data produk yang sesuai.</p>
            </div>
          ) : (
                ...
                {filteredData.map((p) => (
                  <tr key={p.id} className="hover:bg-white/[0.01] transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <span className="px-3 py-1 bg-white/5 border border-white/10 rounded-full text-xs font-medium text-gray-300">{p.satuan}</span>
                    </td>
                    <td className="px-6 py-4">
                      {p.aktif ? (
                        <span className="text-green-400 text-xs font-semibold px-2 py-1 bg-green-500/10 rounded border border-green-500/20">Aktif</span>
                      ) : (
                        <span className="text-red-400 text-xs font-semibold px-2 py-1 bg-red-500/10 rounded border border-red-500/20">Nonaktif</span>
                      )}
                    </td>
          ...
          <div className="bg-[#161b22] border border-white/10 rounded-2xl w-full max-w-md shadow-2xl flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 overflow-y-auto space-y-4">
              <div className="space-y-1.5">
                <Label className="text-gray-300">Nama Produk <span className="text-red-500">*</span></Label>
                <Input value={formData.nama} onChange={e => setFormData({ ...formData, nama: e.target.value })}
                  className="bg-[#0d1117] border-white/10 text-white" placeholder="Ayam Broiler 1.2" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-gray-300">Satuan</Label>
                <select value={formData.satuan} onChange={e => setFormData({ ...formData, satuan: e.target.value })}
                  ...
                  <option value="ekor">ekor (Ekor)</option>
                </select>
              </div>
              <div className="flex items-center justify-between py-2 border-t border-white/5 mt-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500">Produk akan tampil di sistem</p>
                </div>
